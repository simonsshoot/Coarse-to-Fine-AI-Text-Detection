...
def test(opt):
    model_config = RobertaConfig.from_pretrained(opt.model_name,num_labels=2)
    model=RobertaForSequenceClassification.from_pretrained(opt.model_name,config=model_config,ignore_mismatched_sizes=True)
    device="cuda:2"
...
if __name__ == '__main__':
  ...
  opt.model_name="models/RADAR"
  tokenizer = RobertaTokenizer.from_pretrained(opt.model_name,use_fast=False)
  test(opt)